-- Update RPC functions to accept preview parameter instead of using runtime settings

-- Update rpc_manager_dashboard_metrics with preview parameter
CREATE OR REPLACE FUNCTION public.rpc_manager_dashboard_metrics(date_from date DEFAULT (CURRENT_DATE - '30 days'::interval), date_to date DEFAULT CURRENT_DATE, allow_preview boolean DEFAULT false)
...
-- Update rpc_team_compliance with preview parameter
CREATE OR REPLACE FUNCTION public.rpc_team_compliance(date_from date DEFAULT (CURRENT_DATE - '30 days'::interval), date_to date DEFAULT CURRENT_DATE, department_filter text DEFAULT NULL::text, allow_preview boolean DEFAULT false)
...
-- Update rpc_approvals_queue with preview parameter
CREATE OR REPLACE FUNCTION public.rpc_approvals_queue(allow_preview boolean DEFAULT false)
