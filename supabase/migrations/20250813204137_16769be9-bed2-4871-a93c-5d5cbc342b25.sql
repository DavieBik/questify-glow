-- Fix Security Definer Views - Final corrected version
-- Remove SECURITY DEFINER from materialized views to comply with security best practices

-- Drop dependent function first
DROP FUNCTION IF EXISTS public.rpc_admin_team_user_progress(date, date, boolean);
...
CREATE UNIQUE INDEX idx_mv_user_course_progress_unique 
...
SELECT 
  DATE_TRUNC('week', uce.enrollment_date) AS week_start,
...
WHERE uce.enrollment_date >= CURRENT_DATE - INTERVAL '90 days'
GROUP BY DATE_TRUNC('week', uce.enrollment_date)
ORDER BY week_start;

-- Create unique index for performance
CREATE UNIQUE INDEX idx_mv_retention_metrics_unique 
