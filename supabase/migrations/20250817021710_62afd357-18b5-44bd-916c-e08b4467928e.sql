-- Drop existing SCORM RLS policies
DROP POLICY IF EXISTS "Admins and managers can manage SCORM packages" ON public.scorm_packages;
...
DROP POLICY IF EXISTS "Admins and managers can view all SCORM sessions" ON public.scorm_sessions;
DROP POLICY IF EXISTS "Users can manage interactions for their sessions" ON public.scorm_interactions;
DROP POLICY IF EXISTS "Admins and managers can view all SCORM interactions" ON public.scorm_interactions;

-- Create updated SCORM package policies using profiles.role
...
-- Create updated SCORM session policies using profiles.role
...
-- Create updated SCORM interaction policies using profiles.role
