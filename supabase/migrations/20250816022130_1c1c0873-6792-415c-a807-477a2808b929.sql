-- Create RPC functions for manager dashboard

-- Manager dashboard metrics
CREATE OR REPLACE FUNCTION public.rpc_manager_dashboard_metrics(
  date_from date DEFAULT (CURRENT_DATE - '30 days'::interval),
  date_to date DEFAULT CURRENT_DATE
)
RETURNS TABLE(
  overdue_enrollments bigint,
  due_soon_enrollments bigint,
...
-- Approvals queue data (already exists, but let's ensure it's optimized)
