-- Single-tenant migration: Set up organization defaults and triggers (Fixed)
-- This migration prepares the database for single-tenant operation

-- First, temporarily disable the trigger that causes the issue
DROP TRIGGER IF EXISTS on_org_insert_add_admin ON public.organizations;
...
-- Insert the default organization if it doesn't exist
DO $$
DECLARE
...
BEGIN
  ...
  IF existing_org_count = 0 THEN
    -- Insert the default organization without trigger
...
-- Restore the trigger with a safety check
CREATE OR REPLACE FUNCTION public.on_org_insert_add_admin()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
begin
  -- Only add admin if user exists (safety check)
  if v_user is not null and exists(select 1 from auth.users where id = v_user) then
    insert into public.org_members(organization_id, user_id, role)
    values (new.id, v_user, 'admin')
...
CREATE TRIGGER on_org_insert_add_admin
  AFTER INSERT ON public.organizations
  FOR EACH ROW
  EXECUTE FUNCTION public.on_org_insert_add_admin();
