...
-- First, drop the existing materialized views and regular views
...
-- Recreate materialized views without SECURITY DEFINER (they inherit current user's permissions)
...
ALTER TABLE public.mv_course_performance_analytics ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.mv_user_progress_analytics ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.mv_user_course_progress ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.mv_module_analytics ENABLE ROW LEVEL SECURITY;

-- Create RLS policies for materialized views
-- Only admins and managers can access analytics data

CREATE POLICY "Admin and manager access to course analytics" ON public.mv_course_performance_analytics
...
CREATE POLICY "Admin and manager access to user analytics" ON public.mv_user_progress_analytics
    FOR SELECT TO authenticated
    USING (
        EXISTS (
            SELECT 1 FROM public.users u 
...
CREATE POLICY "Admin and manager access to user course progress" ON public.mv_user_course_progress
    FOR SELECT TO authenticated
    USING (
        -- Users can see their own progress OR admins/managers can see all
        user_id = auth.uid() OR
        EXISTS (
            SELECT 1 FROM public.users u 
            WHERE u.id = auth.uid() 
...
CREATE POLICY "Admin and manager access to module analytics" ON public.mv_module_analytics
    FOR SELECT TO authenticated
    USING (
        EXISTS (
            SELECT 1 FROM public.users u 
