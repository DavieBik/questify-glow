-- Fix remaining security issues: view ownership and API exposure

-- 1. Fix view ownership by recreating them under the authenticator role
...
-- 2. Recreate views with proper security context (they will be owned by the current user, not postgres)
-- Note: These will now respect RLS policies of the underlying tables
CREATE VIEW public.v_learning_patterns 
SECURITY INVOKER AS
...
CREATE VIEW public.v_module_metrics 
SECURITY INVOKER AS
...
CREATE VIEW public.v_skills_gap 
SECURITY INVOKER AS
